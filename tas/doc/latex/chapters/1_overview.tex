...
Both motors bring its own controllers which take a PWM-signal as input to set the desired revolution rate or angle. The signal is shown in picture \ref{fig:pwm_signal_motors}. The input signals are provided by a little Arduino board (see next section\refsec{overview_arduino}).
...
\begin{center}
\begin{tikzpicture} 
\begin{axis}[xmin=-1000, xmax=2000, ymin=-1, ymax=1]
 \addplot gnuplot 
...
To provide the signals to the motor controllers there is a little Arduino board on the car which can be seen in picture \reffig{}. It receives the PWM-values from the mainboard of the car via USB and sets them to the output pins which are connected to the controllers.
...
\todo{Correct entries in table (fake values)}
