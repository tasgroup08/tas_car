...
Power supply for the brushless motor can disconnected by a switch, which you can see in picture \reffig{} on the top. With the switch shown on the bottom you can select the source of the input signals for the brushless motor controller:

\begin{itemize}
\item Position 1:\\
In this position the input signal is taken from the ROS \texttt{/servo} topic (see section \refsec{} for more information).
\item Position 2:\\
In this positon the input signal is taken from the receiver for the original remote controller (see section \refsec{} for more information).
\end{itemize}
...
\end{figure}
To supply the board from battery, there is a 12V DC-DC Converter which you can see in picture \reffig{}. Just connect the battery and the converter like in the picture and the plug the 24-pin connector into the board.
