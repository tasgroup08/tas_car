...
//const Vector3::ConstPtr& rpy_msg;
...
// compensate for yaw drift

float drift=0.00336075452 ;//yaw drift in rad/sec (gerty) (change for each car or create calibrate mode)

tf::Quaternion quat_biased(q1, q2, q3, q0);


double ouryaw;
ouryaw= tf::getYaw(quat_biased);//calculate yaw from quaternion

Yaw=ouryaw-drift*dt;//subtract drift per second
tf::Quaternion quat_unbiased = tf::createQuaternionFromYaw(Yaw)//create unbiased Quaternion

//try this instead rotate the quaternion about yaw axis by dt times drift
//double Yawdif;
//Yawdif=-drift*dt;
//tf::Quaternion reverse_drift_quat = tf::createQuaternionFromYaw(Yawdif);

//tf::Quaternion quat_unbiased;
//quat_unbiased=reverse_drift_quat*quat_biased; Rotation of quat biased about yaw quaternion multiplication is implemented

q1 = quat_unbiased.getX();
q2 = quat_unbiased.getY();
q3 = quat_unbiased.getZ();
q0 = quat_unbiased.getW();
